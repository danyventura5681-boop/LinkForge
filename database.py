...
class User(Base):
    ...
    accepted_privacy = Column(Integer, default=0)  # 0 = no aceptó, 1 = sí aceptó
...
def create_user(telegram_id: int, username: str = None, referred_by: int = None):
    ...
    try:
        ...
        user = User(
            ...
            accepted_privacy=0  # Por defecto no ha aceptado políticas
        )
        session.add(user)
...
# ===========================================
# NUEVAS FUNCIONES PARA POLÍTICA DE PRIVACIDAD
# ===========================================

def set_user_accepted_privacy(telegram_id: int):
    """Marca que el usuario aceptó la política de privacidad."""
    session = SessionLocal()
    try:
        user = session.query(User).filter_by(telegram_id=telegram_id).first()
        if user:
            user.accepted_privacy = 1
            session.commit()
            logger.info(f"✅ Usuario {telegram_id} aceptó política de privacidad")
        else:
            logger.warning(f"⚠️ Usuario {telegram_id} no encontrado para marcar política")
    except Exception as e:
        logger.error(f"❌ Error en set_user_accepted_privacy({telegram_id}): {e}")
        session.rollback()
    finally:
        session.close()

def has_user_accepted_privacy(telegram_id: int) -> bool:
    """Verifica si el usuario aceptó la política de privacidad."""
    session = SessionLocal()
    try:
        user = session.query(User).filter_by(telegram_id=telegram_id).first()
        if user:
            result = user.accepted_privacy == 1
            logger.info(f"✅ has_user_accepted_privacy: Usuario {telegram_id} ha aceptado: {result}")
            return result
        else:
            logger.info(f"✅ has_user_accepted_privacy: Usuario {telegram_id} no existe, retorna False")
            return False
    except Exception as e:
        logger.error(f"❌ Error en has_user_accepted_privacy({telegram_id}): {e}")
        return False
    finally:
        session.close()
