...
async def process_referral(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.info(f"🔍 Referente encontrado: {referrer_id}")
        ...
        if existing_user.referred_by is None:
            ...
            try:
                ...
                if user_obj:
                    ...
                    # Obtener total de referidos actualizado
                    total_referidos = get_referrals_count(referrer_id)

                    # ✅ NOTIFICAR AL REFERENTE (MEJORADA)
                    bot_username = (await context.bot.get_me()).username
                    try:
                        await context.bot.send_message(
                            chat_id=referrer_id,
                            text=f"🎉 **¡Alguien confirmó tu referencia!** 🎉\n\n"
                                 f"👤 Usuario: @{username}\n"
                                 f"🎁 **+50 reputación** añadida a tu cuenta.\n\n"
                                 f"📊 Ahora tienes **{total_referidos}** referido(s).\n\n"
                                 f"💰 Reputación total ganada por referidos: **+{total_referidos * 50}**\n\n"
                                 f"🔗 Tu enlace: https://t.me/{bot_username}?start=ref_{referrer_id}\n\n"
                                 f"💡 Sigue compartiendo tu enlace para ganar más reputación.",
                            reply_markup=InlineKeyboardMarkup([
                                [InlineKeyboardButton("👥 Ver mis referidos", callback_data="referral")],
                                [InlineKeyboardButton("🏠 Ir al Menú", callback_data="volver_menu")]
                            ]),
                            parse_mode='Markdown'
                        )
                    ...
                    # Mensaje de confirmación al usuario existente
                    try:
                        await update.message.reply_text(
                            "✅ **¡Referido confirmado!** 🎉\n\n"
                            f"📊 Gracias por unirte con el enlace de invitación.\n\n"
    ...
    if user_created:
        ...
        try:
            await context.bot.send_message(
                chat_id=referrer_id,
                text=f"🎉 **¡Nuevo usuario por tu enlace!** 🎉\n\n"
                     ...
                     f"🔗 Tu enlace: https://t.me/{bot_username}?start=ref_{referrer_id}\n\n"
                     f"💡 Sigue compartiendo tu enlace para ganar más reputación.\n\n"
                     f"🏆 ¡Entre más invites, más beneficios tendrás!",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("👥 Ver mis referidos", callback_data="referral")],
                    [InlineKeyboardButton("🏠 Ir al Menú", callback_data="volver_menu")]
                ]),
        ...
        try:
            await update.message.reply_text(
                ...
                f"📊 Te has unido gracias a la invitación de un amigo.\n\n"
                ...
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🏠 Ir al Menú Principal", callback_data="volver_menu")]]),
                parse_mode='Markdown'
            )
        except Exception as e:
